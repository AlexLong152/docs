...
\begin{figure}[h!]
  \begin{center}
    ...
    \caption{Kinematics in the center of mass frame and quantum
      ...
      processes$\hat{O}_{12}$, bottom: three-body processes$\hat{O}_{123}$. Green represents the
...
$\HeT$ and $\HeF$ but to extend this $\LiS$ ($A=6$) involves many body
...
outside of a certain cutoff $\Lambda$, and therefore neglect it in our calculation.
...
however have dependence on physical momenta and therefore we cannot directly use an SRG 
...
This is of much interest experimentally since this target is solid at room temperature and does not decay, it is therefore 
easy to place in a beam path.
\ques{$\LiS$ results go here maybe?}
...
\end{equation}
\ques{Can these parameters be extracted experimentally?}
There are many experiments on $\LiS$, but as of yet there is no theory prediction, we seek to fill this gap \cite{60MeV,86MeV}.
...
\ques{I don't want to include that diagram since I stole the figures from
...
The two-body contributions do not easily decompose into multipole interactions, therefore we preform the calculation through expansions in the chiral Lagrangian and 
At threshold energy this reaction kernel has been analyzed by Lenkewitz \etal \cite{L2011, L2013}.
We must extend this to finite energy, and to higher order if necessary.
...
The pion-pion scattering kernel is similar to the pion photoproduction kernel. Beane \etal has developed this kernel at threshold for both one-body and two-body interactions \cite{Beane2003}.
We may extend this to finite energy. 
When the pion-photoproduction and pion-pion scattering kernels have successfully been developed, we will instantly be able to calculate all of these reactions on targets previously analyzed in the density formalism since we already have produced the densities required.
In particular, we will calculate all of these reactions with the targets $\HeT$, $\HeF$, and $\LiS$.
\section{Conclusion}
